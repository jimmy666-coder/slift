...
const METRICS = [
  ...
  },
  {
    key: "energy",
    label: "Energy level",
    hint: "1 = drained, 10 = fully charged",
  },
  {
    key: "motivation",
    label: "Motivation",
    hint: "1 = low, 10 = unstoppable",
  },
];
...
function getAccentOpacity(value) {
  return 0.14 + (value / 10) * 0.34;
}
    ...
    <div style={styles.metricBlock}>
      <div style={styles.metricHeader}>
        ...
        <div
          style={{
            ...styles.metricValue,
            color: value >= 7 ? COLORS.accent : COLORS.text,
  ...
  return (
    <button
      ...
      style={{
        ...
        background: selected
          ? "linear-gradient(180deg, rgba(0,255,156,0.10) 0%, rgba(123,63,242,0.10) 100%)"
...
export default function MorningCheckin({
  ...
  const mergedInitialValues = useMemo(
    () => ({
      ...
      ...initialValues,
      muscleGroups:
        Array.isArray(initialValues?.muscleGroups) &&
        initialValues.muscleGroups.length > 0
          ? initialValues.muscleGroups.slice(0, 3)
          : defaultValues.muscleGroups,
    }),
    [initialValues]
  );
  ...
  const toggleMuscleGroup = (group) => {
    setForm((prev) => {
      ...
      if (exists) {
        const nextGroups = prev.muscleGroups.filter((item) => item !== group);
        return {
          ...prev,
          muscleGroups: nextGroups.length > 0 ? nextGroups : [],
        };
      }

      if (prev.muscleGroups.length >= 3) {
        return prev;
      }
      ...
      <div style={styles.container}>
        <div style={styles.topBlock}>
          ...
          <p style={styles.subtitle}>
            Quick check-in. Clean input, better training. Small ritual, better
            output. Miraculous, I know.
          </p>
        </div>
            ...
            <div style={styles.metricsGrid}>
              {METRICS.map((metric) => (
                <SliderRow
                  ...
                  active={activeSlider === metric.key}
                  onActiveChange={(isActive) =>
                    setActiveSlider(isActive ? metric.key : null)
                  }
          ...
          <section style={styles.card}>
            ...
            <div style={styles.buttonGrid}>
              {MUSCLE_GROUPS.map((group) => {
                ...
                const limitReached =
                  !selected && form.muscleGroups.length >= 3;
...
const styles = {
  ...
  selectButton: {
    ...
    cursor: "pointer",
    transition:
      "transform 140ms ease, opacity 140ms ease, border-color 180ms ease, background 180ms ease, color 180ms ease",
