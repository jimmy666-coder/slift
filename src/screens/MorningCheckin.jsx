...
export function MorningCheckin({ onComplete, profile }) {
  ...
  const [duration, setDuration] = useState(profile?.duration || '60 min')
  ...
  function handleSubmit() {
    ...
    onComplete({ ...values, muscleGroups: selectedGroups, duration, trainingStyle, score })
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.groupGrid}>
          {MUSCLE_GROUPS.map(group => (
            <button key={group} onClick={() => toggleGroup(group)} style={{
              ...styles.groupBtn,
              background: selectedGroups.includes(group) ? '#7c3aed' : '#0f0f1a',
              border: selectedGroups.includes(group) ? '2px solid #8b5cf6' : '2px solid #2a2a4a',
              color: selectedGroups.includes(group) ? 'white' : '#9ca3af',
            }}>
              {group}
            </button>
          ))}
        ...
        <p style={styles.cardHint}>What type of session today?</p>
        <div style={styles.groupGrid}>
          {STYLES.map(s => (
            <button key={s} onClick={() => setTrainingStyle(s)} style={{
              ...styles.groupBtn,
